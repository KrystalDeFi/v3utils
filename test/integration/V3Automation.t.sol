...
contract V3AutomationIntegrationTest is IntegrationTestBase {
    ...
    function testAutoAdjustRangeWithPermit() external {
        _increaseLiquidity();
        (address userAddress, uint256 privateKey) = makeAddrAndKey("positionOwnerAddress");
        ...
        bytes memory signature = _signOrder(emptyUserConfig, privateKey);
        uint256 countBefore = NPM.balanceOf(userAddress);
        (uint256 nonce, , , , , , , uint128 liquidityBefore, , , , ) = NPM.positions(
            TEST_NFT
        );
        ...
        V3AutomationHarness.Permit memory permitMessage = V3AutomationHarness.Permit(
            address(v3automation),
            TEST_NFT,
            nonce,
            params.deadline
        );
        bytes32 digest = v3automation.getTypedDataHash(NFPM_DOMAIN_SEPARATOR, v3automation.hash(permitMessage));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(privateKey, digest);

        // using permit / execute pattern
        vm.prank(TEST_OWNER_ACCOUNT);

        v3automation.executeWithPermit(params, v, r, s);(params);
